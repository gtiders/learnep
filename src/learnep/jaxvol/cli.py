...
def active(
    train_path: Annotated[
        str, typer.Option(help="Path to training data")
    ] = "train.xyz",
    ...
    mode: Annotated[
        str, typer.Option(help="Selection mode (adaptive/maxvol)")
    ] = "adaptive",
    device: Annotated[
    ...
    gamma_min: Annotated[
        Optional[float],
        typer.Option(help="Min gamma threshold (Deprecated in adaptive mode)"),
    ] = None,
    gamma_max: Annotated[
        Optional[float],
        typer.Option(help="Max gamma threshold (Deprecated in adaptive mode)"),
    ] = None,
):
    """
    ...
    _, active_structs = get_active_set(
        B_projections, B_indices, write_asi=True, batch_size=batch_size, mode=mode
    )
...
def extend(
    ...
    _, active_structs = get_active_set(
        B_projections, B_indices, write_asi=False, batch_size=batch_size, mode=mode
    )
...
def gamma(
    ...
    nep_path: Annotated[str, typer.Option(help="Path to nep.txt")] = "nep.txt",
    asi_path: Annotated[
        str, typer.Option(help="Path to active set inverse")
    ] = "active_set.asi",
    output: Annotated[str, typer.Option(help="Output file")] = "large_gamma.xyz",
    threshold: Annotated[
        float, typer.Option(help="Min Gamma threshold for selection")
    ] = 1.0,
