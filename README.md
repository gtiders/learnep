...
*   **JaxVol 深度集成**: 使用 JAX 加速的自适应采样，自动处理从秩亏（Rank-Deficient）到满秩（Full-Rank）的过渡。
*   **候选集去重 (Candidate Resampling)**: 基于 MaxVol 二次筛选逻辑，自动识别并剔除 MD 轨迹中的高度冗余帧，确保选出的结构信息量最大化。
...
*   `gamma`:
    *   `threshold`: Gamma 粗筛阈值。
    *   `threshold_max`: 熔断阈值，超过此值会触发爆炸检测并停止扫描，防止非物理结构进入。
    *   `n_max_label`: 每轮最大送去 VASP 计算的结构数量。
  *   **二次筛选逻辑**: 系统会自动执行 `Combine(Train + Candidates) -> MaxVol Sub-select -> Random Sampling` 的优化流程。
...
4.  **筛选阶段 (Selection)**:
    ...
    *   使用 JaxVol 扫描 MD 轨迹，计算每个帧的 Gamma 值并进行初步粗筛。
    *   **二次筛选 (核心提升)**: 将高分候选者与训练集进行联合 MaxVol 筛选，剔除彼此相似的结构。
    *   如果剩余结构仍多于 `n_max_label`，则进行随机抽样，生成最终的 `candidates.xyz`。
5.  **标号阶段 (Label)**:
