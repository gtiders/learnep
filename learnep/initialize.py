...
def initialize_workspace(config: Config, logger: logging.Logger) -> None:
    ...
    if insufficient_elements:
        ...
        logger.warning("  如果 MaxVol 失败，将使用冷启动模式")

    # 检查是否数据足够运行 MaxVol
    bootstrap_mode = False
    try:
        from .maxvol import check_data_sufficient

        is_sufficient, stats = check_data_sufficient(train_structures, str(nep_dst))

        if not is_sufficient:
            logger.info("\n  检测到训练数据不足以运行 MaxVol，将使用冷启动模式")
            for elem, (count, dim) in stats.items():
                status = "✓" if count >= dim else "✗"
                logger.info(f"    {status} 元素 {elem}: {count}/{dim} 原子环境")
            bootstrap_mode = True
    except Exception as e:
        logger.warning(f"  数据充足性检测失败: {e}，将尝试运行 MaxVol")
    ...
    if not bootstrap_mode:
        try:
            ...
            logger.warning(f"  活跃集生成失败: {e}")
            logger.info("  将使用冷启动模式（不需要活跃集）")
            bootstrap_mode = True

    # =========================================================================
    # 步骤 4: 准备 GPUMD 探索任务（仅在正常模式下）
    # =========================================================================
    if not bootstrap_mode:
        logger.info("\n步骤 4: 准备 GPUMD 探索任务")
        ...
        logger.info("\n步骤 4: 跳过 GPUMD 准备（冷启动模式）")
        logger.info("  冷启动模式将在迭代运行时自动使用冷启动配置")
    ...
    if bootstrap_mode:
        logger.info("\n*** 冷启动模式 ***")
        logger.info("由于训练数据不足，将使用冷启动模式运行")
        logger.info("冷启动模式会自动：")
        logger.info("  1. 使用 dump_exyz 保存 MD 轨迹")
        logger.info("  2. 过滤不合理的结构")
        logger.info("  3. 使用 FPS 选择多样性结构")
        logger.info("  4. 数据足够后自动切换到正常模式")
    else:
        logger.info("\n下一步: 提交 GPUMD 探索任务")
